...
absentee_flu = absentee_flu[, pre := ifelse(
  test = absentee_flu$schoolyr != "2014-15" &
    absentee_flu$schoolyr != "2015-16" &
    absentee_flu$schoolyr != "2016-17" &
    absentee_flu$schoolyr != "2017-18",
  yes = 1,
  no = 0
)]

absentee_flu = absentee_flu[, .(dist, absent_ill, absent_all, schoolyr, pre, school)]

absentee_flu = absentee_flu[complete.cases(absentee_flu),]

# create variable for pre + schoolyr
absentee_flu = absentee_flu[, schoolyrp := ifelse(pre == 1, "Pre", as.character(schoolyr))]
absentee_flu = absentee_flu[, schoolyrp := factor(schoolyrp,
...
mn.abs.l = mn.abs %>% 
  gather(abstype, mean, mn_all:mn_ill) %>%
  mutate(mean = mean * 100)

pdf(
  file = paste0(fig_dir, "means-yr.pdf"),
  width = 7,
  height = 3
)
...
pdf(
  file = paste0(fig_dir, "mean-diff-yr-crude.pdf"),
  width = 7,
  height = 3
)
...
write_rds(
  ...
  path = paste0(res_dir, "tables/", "1-unadj-table-raw.RDS")
)
